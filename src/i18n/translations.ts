// Translation files for FR/EN
export const translations = {
  fr: {
    // ProfilePage
    settings: "Paramètres",
    loading: "Chargement...",
    premium: "Premium",
    premiumSub: "Toutes les fonctionnalités débloquées",
    premiumPrice: "4,99 €/mois",
    proPrice: "14,99 €/mois",
    subscribe: "S'abonner",
    subscriptionActive: "Actif",
    subscriptionUntil: "Valable jusqu'au",
    subscriptionUntilUnknown: "Abonnement actif (date de fin non renseignée)",
    cancelSubscription: "Se désabonner",
    cancelSubscriptionConfirm: "Confirmer la résiliation de cet abonnement ? L'accès Premium/Pro sera désactivé immédiatement.",
    subscriptionCancelledPremium: "Abonnement Premium résilié.",
    subscriptionCancelledPro: "Abonnement Pro résilié.",
    subscriptionTotal: "Total",
    subscriptionDuration: "Durée de l'abonnement",
    subscriptionMonthsCount: "{{n}} mois",
    subscriptionPerMonth: "/ mois",
    payWithCardBeta: "Gratuit en test 😉",
    cardholderName: "Titulaire de la carte",
    cardholderPlaceholder: "Nom sur la carte",
    cardNumber: "Numéro de carte",
    cardExpiry: "Expiration",
    cardCvc: "CVC",
    payWithCard: "Payer par carte",
    paymentProcessing: "Paiement en cours…",
    paymentSecureNote: "Paiement simulé — prêt pour branchement Stripe ou PayPlug.",
    paymentFailed: "Le paiement a échoué. Réessayez.",
    paymentSuccessPremium: "Premium activé. Merci !",
    paymentSuccessPro: "Compte Pro activé. Merci !",
    cardholderRequired: "Indiquez le titulaire de la carte.",
    cardNumberInvalid: "Numéro de carte invalide.",
    cardExpiryInvalid: "Date d'expiration invalide.",
    cardCvcInvalid: "Code CVC invalide.",
    adminMode: "Mode Admin",
    adminModeSub: "Accès aux outils de modération",
    darkMode: "Mode sombre",
    darkModeSub: "Interface sombre (activé par défaut)",
    adminInfoTab: "Info",
    adminInfoSkipEmailLabel: "Inscription sans vérification email",
    adminInfoSkipEmailSub:
      "Valide automatiquement les nouveaux comptes (utile si l'envoi d'email n'est pas configuré)",
    adminInfoSplashLabel: "Splash screen au démarrage",
    adminInfoSplashSub: "Afficher l'écran « Happy, let's GO » à l'ouverture de l'app",
    adminInfoSplashImageLabel: "Image du splash",
    adminInfoSplashImageHint:
      "Visible par tous au démarrage. L'URL est synchronisée via app_config.",
    adminInfoSplashImageUpload: "Changer l'image",
    adminInfoSplashImageUploading: "Envoi…",
    adminInfoSplashImageReset: "Image par défaut",
    adminInfoSplashImageSaved: "Image du splash enregistrée",
    adminInfoModalLabel: "Modale d'information",
    adminInfoModalSub: "Afficher un message à tous les utilisateurs connectés",
    adminInfoModalDismissLabel: "Fermeture autorisée",
    adminInfoModalDismissSub: "Afficher une croix pour fermer la modale",
    adminInfoMessageLabel: "Message pour tous",
    adminInfoMessagePlaceholder:
      "Mises à jour, événements importants, maintenance…",
    adminInfoMessageHint:
      "Rédigez le texte puis publiez pour l'envoyer à tous les utilisateurs.",
    adminInfoPublish: "Publier à tous",
    adminInfoPublished: "Message publié",
    adminInfoForceReloadLabel: "Imposer une mise à jour",
    adminInfoForceReloadSub:
      "À la publication, demande à tous de recharger l'application",
    announcementModalTitle: "Information",
    appForceReloadTitle: "Mise à jour requise",
    appForceReloadBody:
      "Une nouvelle version de l'application est disponible. Rechargez pour rester à jour.",
    appForceReloadAction: "Recharger l'application",
    adminEditProfile: "Modifier le profil (admin)",
    adminDeleteProfile: "Supprimer l'utilisateur",
    adminDeleteProfileConfirm:
      "Supprimer définitivement cet utilisateur ? Il disparaîtra des suggestions, visites et discussions privées.",
    adminDeleteEvent: "Supprimer la sortie",
    adminDeleteEventConfirm:
      "Supprimer définitivement cette sortie ? Elle et son groupe de discussion seront effacés.",
    adminDeleteConversation: "Supprimer la discussion",
    adminDeleteConversationConfirm:
      "Supprimer définitivement cette discussion et tous ses messages ?",
    professional: "Compte professionnel",
    professionalSub: "Organisateur d'activités et d'événements pro",
    calendarTab: "Calendrier",
    previousMonth: "Mois précédent",
    nextMonth: "Mois suivant",
    noEventsOnSelectedDay: "Aucune sortie créée ce jour-là.",
    remindButton: "Relancer",
    reminderSent: "Relance envoyée",
    reminderRead: "Lu",
    reminderUnread: "Envoyé",
    remindAllButton: "Tout relancer",
    myProAgenda: "Mon Agenda Professionnel",
    language: "Langue",
    clearCache: "Vider le cache",
    clearCacheButton: "Vider",
    clearCacheConfirm:
      "Vider le cache local et recharger les données depuis le serveur ?",
    clearCacheSuccess: "Cache vidé. Données rechargées.",
    clearCacheError: "Impossible de vider le cache.",
    betaFeatures: "Activer les fonctionnalités Bêta",
    howToInstall: "Comment installer",
    howToInstallTitle: "Installer l'application",
    howToInstallIos: "Sur iPhone (Safari)",
    howToInstallAndroid: "Sur Android",
    infoButton: "Info",
    logout: "Se déconnecter",
    deleteAccount: "Supprimer mon compte",

    // Profile tabs
    favoritesCreated: "Mes sorties",
    friends: "Amis",
    reports: "Signalements",
    history: "Passés",
    notifications: "Notifications",

    // Profile sections
    badges: "Badges",
    badgeAddPlaceholder: "Nouveau badge…",
    badgeAddAria: "Ajouter un badge",
    badgeRemoveAria: "Retirer le badge",
    badgeAdminHint: "Mode admin : cliquez sur × pour retirer, + pour ajouter un badge.",
    badgeCatalogTitle: "Catalogue de badges",
    badgeQuickAddTitle: "Ajout rapide",
    badgeCatalogAdd: "Nouveau badge",
    badgeCatalogAddPlaceholder: "Nom du badge…",
    badgeEditAria: "Modifier le badge",
    badgeRemoveCatalogAria: "Retirer du catalogue",
    reliability: "Fiabilité",
    upcoming: "À venir",
    upcomingOutings: "Sorties à venir",
    noShows: "No-shows",

    // Bio card
    memberSince: "Membre depuis 2024",
    editProfile: "Modifier le profil",
    cancel: "Annuler",
    save: "Enregistrer",
    verified: "Profil vérifié",
    adminProVerifiedTitle: "Vérification Pro",
    adminProVerifiedSub: "Activé manuellement par un admin — pastille visible sur l’annuaire Pro.",

    // Empty states
    noFavoritesUpcoming:
      "Aucune sortie à venir : favoris, organisées par vous ou auxquelles vous participez. Les dates passées sont dans « Passés ».",
    noFriends: "Aucun ami pour le moment.",
    noReports: "Aucun signalement à traiter.",
    noNotifications: "Aucune notification.",
    notifParticipantJoinedTitle: "Nouveau participant",
    notifParticipantJoinedBody: "{name} s'inscrit à « {title} ».",
    notifParticipantLeftTitle: "Désinscription",
    notifParticipantLeftBody: "{name} s'est désinscrit(e) de « {title} ».",
    notifWaitlistJoinedTitle: "Liste d'attente",
    notifWaitlistJoinedBody: "{name} rejoint la liste d'attente de « {title} ».",
    notifWaitlistLeftTitle: "Liste d'attente",
    notifWaitlistLeftBody: "{name} a quitté la liste d'attente de « {title} ».",
    notifWaitlistAcceptedTitle: "Demande acceptée",
    notifWaitlistAcceptedBody: "{name} a accepté votre demande pour « {title} ».",
    notifWaitlistRejectedTitle: "Demande refusée",
    notifWaitlistRejectedBody: "{name} a refusé votre demande pour « {title} ».",
    noPastActivity: "Aucune activité passée.",

    // Reports
    suspiciousLogin: "Connexion suspecte",
    profile: "Profil",
    chat: "Messages",
    navChat: "Chat",
    navSortie: "Évènements",
    pro: "Pro",
    professionalsTitle: "Professionnels",
    proSearchPlaceholder: "Rechercher nom, prénom, ville, description…",
    proFilterLabel: "Filtrer par métier",
    proJobLabel: "Métier",
    proJobPlaceholder: "Ex. Sophrologue, massage bien-être, coiffure à domicile…",
    proCategoryTypeLabel: "Type de métier",
    proCategoryTypeHint: "Catégorie affichée dans l’annuaire et utilisée pour le filtre.",
    proCategoryLabel: "Métier",
    proFilterAll: "Tous",
    proNoResults: "Aucun professionnel ne correspond à votre recherche.",
    proViewModeLabel: "Mode d'affichage",
    proViewList: "Liste",
    proViewMap: "Carte",
    proClientsLabel: "Clients",
    proContactButton: "Contacter",
    proContactSoon: "La messagerie pro arrive bientôt.",
    proWebsiteLabel: "Site web",
    proSocialLabel: "Réseaux sociaux",
    proPhoneLabel: "Téléphone",
    proWebsitePlaceholder: "https://www.mon-site.fr",
    proSocialPlaceholder: "https://instagram.com/moncompte",
    proPhonePlaceholder: "+33 6 12 34 56 78",
    proAddressLabel: "Adresse du cabinet",
    proAddressPlaceholder: "1 Place du Capitole, 31000 Toulouse",
    proAddressHint: "Numéro, rue, code postal et ville (les espaces sont acceptés). Validée à l’enregistrement pour la carte Pro.",
    proAddressFormatHint: "Adresse trop courte. Ex. : 1 Place du Capitole, 31000 Toulouse",
    proAddressGeocoding: "Validation…",
    proAddressInvalid: "Adresse introuvable. Vérifiez rue, code postal et ville.",
    proAddressValidated: "Adresse validée — visible sur la carte Pro.",
    event: "Sortie",
    dismiss: "Ignorer",
    hideAndNotify: "Masquer et informer",

    // Notifications
    invitationSent: "Invitation envoyée",
    willNotify: "recevra une notification pour rejoindre",

    // Settings
    photoUploadFailed: "Échec de l'envoi de la photo",
    photoUploading: "Envoi de la photo…",
    changePhoto: "Change photo",

    // HomePage
    searchActivity: "Rechercher une activité…",
    topActivities: "Top 5 Activités",
    activitiesSchedule: "Agenda des activités",
    all: "Tout",
    sport: "Sport",
    outings: "Sorties",
    culture: "Culture",

    // EventCard
    beta: "Bêta",
    registered: "Inscrit",
    register: "S'inscrire",
    organizer: "Organisateur",
    pending: "En attente",
    addFavorite: "Ajouter aux favoris",
    removeFavorite: "Retirer des favoris",
    you: "Vous",
    viewing: "En cours de visualisation",
    available: "Disponible",
    full: "Complet",

    // EventDetailPage
    confirmUnregister:
      "Voulez-vous vraiment vous désinscrire de cette activité ?",
    aboutEvent: "À propos de l'activité",
    waitlist: "Liste d'attente",
    proposedBy: "Proposé par",
    linkCopied: "Lien copié dans le presse-papier !",
    karmaShort: "Karma",
    karmaPointsLabel: "{count} points karma",
    validatePresenceAriaLabel: "Valider la présence (+2 karma)",
    rateOrganizerTitle: "Comment s'est passée la sortie ?",
    rateOrganizerSubtitle: "Votre note influence le karma de l'organisateur.",
    rateOrganizerGood: "Bien",
    rateOrganizerBad: "Mauvais",
    rateOrganizerGoodAria: "Noter positivement l'organisateur",
    rateOrganizerBadAria: "Noter négativement l'organisateur",
    rateOrganizerThanks: "Merci pour votre retour !",
    createEventKarmaCost: "−{cost} karma",
    createEventKarmaFree: "Gratuit (Pro)",
    createEventKarmaReward: "+{reward} karma si sortie réussie (≥ moitié inscrits)",
    shareLinkTooltip: "Cliquer pour copier le lien",
    participants: "Participants",
    unregister: "Se désinscrire",
    joinWaitlist: "Rejoindre la liste d'attente",
    leaveWaitlist: "Quitter la liste d'attente",
    approveWaitlistEntry: "Accepter la demande",
    rejectWaitlistEntry: "Refuser la demande",
    share: "Partager",
    report: "Signaler",
    yes: "Oui",
    no: "Non",

    // CreateEventPage
    eventTitlePlaceholder: "Indiquez un titre pour votre événement.",
    locationPlaceholder: "Indiquez un lieu ou un point de rendez-vous.",
    participantsError: "Le nombre de participants doit être entre 1 et 100.",
    imageUploadFailed: "Échec de l'envoi de l'image.",
    createEvent: "Créer la sortie",
    title: "Titre",
    location: "Lieu",
    date: "Date",
    time: "Heure",
    maxParticipants: "Participants max",
    description: "Description",
    eventPriceLabel: "Tarif",
    eventPriceFree: "Gratuit",
    eventPricePaid: "Payant",
    eventPricePlaceholder: "Prix par personne",
    eventJoinTipEnable: "Proposer un Billet de confiance",
    eventJoinTipRequired: "Billet de confiance obligatoire (sortie payante)",
    eventJoinTipHint:
      "Frais de réservation pour l'application : les participants paient ce montant pour confirmer leur inscription à la sortie.",
    eventJoinTipAmountLabel: "Montant du Billet de confiance",

    // ChatPage
    newGroup: "Nouveau groupe",
    addGroup: "+ Chat",
    createGroupNamePrompt: "Nom du groupe :",
    createGroupNameRequired: "Indiquez un nom pour le groupe.",
    noChatRooms: "Aucune conversation pour le moment.",
    favoriteChatConversations: "Conversations favoris",

    // ChatRoomPage
    members: "membres",
    directMessage: "Message direct",
    view: "Voir",
    messageInputPlaceholder: "Écrivez un message…",
    sendMessage: "Envoyer",

    // ChatSettingsPage
    chatSettings: "PARAMÈTRES DE DISCUSSION",
    muteNotifications: "Couper les sons",
    blockNotifications: "Bloquer les notifications",
    leaveGroup: "Quitter le groupe",
    deleteGroup: "Supprimer le groupe",
    allFriendsInGroup: "Tous vos amis sont déjà dans le groupe.",

    // QuestionnaireModal
    howAreYou: "Comment ça va ?",
    step: "ÉTAPE",
    of: "SUR",
    step1: "ÉTAPE 1 SUR 3",
    step2: "ÉTAPE 2 SUR 3",
    step3: "ÉTAPE 3 SUR 3",
    continue: "Continuer",
    skip: "Passer",
    questionPlaceholder: "Écrivez quelque chose ici...",
    badges_punctual: "Ponctuel",
    badges_organizer: "Organisateur",
    badges_friendly: "Amical",
    badges_explorer: "Explorateur",

    // ReportModal
    reportExplanation: "Décrivez ce qui vous semble inapproprié ou dangereux.",
    explanation: "Explication",
    required: "(obligatoire)",
    exampleOffensive: "Exemple : contenu offensant…",
    describeIssue: "Veuillez décrire le problème.",
    send: "Envoyer",

    // OtherProfilePage
    noBio: "Pas de bio pour le moment.",
    events: "Événements",
    pioneer: "Pionnier",

    // Form inputs
    name: "Nom",
    age: "Âge",
    cityPlaceholder: "Ville (ex. Toulouse)",
    bio: "Bio...",
    placeholder: "Bio...",

    // Navigation
    back: "Retour",
    close: "Fermer",

    // Aria labels & buttons
    settingsAriaLabel: "Paramètres",
    notificationSoundEnable: "Activer le son des notifications",
    notificationSoundDisable: "Désactiver le son des notifications",
    searchAriaLabel: "Rechercher",
    previousWeek: "Semaine précédente",
    nextWeek: "Semaine suivante",
    calendarAriaLabel: "Calendrier",
    clearSearch: "Effacer",
    validateSearch: "Valider",
    backToTextSearch: "Retour à la recherche texte",
    dateFilterAriaLabel: "Filtrer par date (à partir de cette date)",
    locationFilterAriaLabel: "Filtrer par lieu",
    themeFilterAriaLabel: "Filtrer par thème (même liste que les couvertures)",
    filtersAriaLabel: "Filtres : date, lieu, tag",
    createEventAriaLabel: "Créer un événement",
    topWeeklyEvents: "Top 5 de la semaine",
    noEventsThisWeek: "Aucun événement sur cette semaine.",
    noSearchResults: "Aucune activité ne correspond à votre recherche.",
    eventsSearchLoadingMore: "Chargement des sorties suivantes…",
    searchEvents: "Recherche",

    // CreateEventPage aria-labels
    themeCoversSuggestions: "Couvertures suggérées par thème",
    addCoverPhoto: "Ajouter une photo de couverture",
    increaseMax: "Augmenter le maximum",
    decreaseMax: "Diminuer le maximum",
    privateEventToggle: "Sortie privée",
    privateEventHint:
      "Masquée de l'agenda public ; visible par vous et les inscrits",
    aboutSectionTitle: "À PROPOS",
    optionsSectionTitle: "OPTIONS",
    createEventParticipantsRangeAlert:
      "Le nombre de participants doit être entre {min} et {max}.",
    createEventErrorTitle: "Indiquez un titre pour votre événement.",
    createEventErrorLocation: "Indiquez un lieu ou un point de rendez-vous.",
    createEventErrorPrice: "Indiquez un prix valide (nombre entier supérieur à 0) ou choisissez Gratuit.",
    createEventErrorNotFound: "Cette sortie est introuvable.",
    createEventErrorGeneric:
      "Impossible de créer la sortie. Vérifiez les champs et réessayez.",
    createEventKarmaEnrollmentHint:
      "Bonus seulement si au moins la moitié des places sont inscrites",
    deleteEvent: "Annuler la sortie",
    editEvent: "Modifier la sortie",
    newEvent: "Nouvelle sortie",
    saveChanges: "Enregistrer les modifications",
    createEventButton: "Créer l'événement",
    createEventInviteAll: "Tous",
    createEventInviteNone: "Aucun",
    createEventInviteSelectedOne: "1 ami sélectionné",
    createEventInviteSelectedMany: "{{n}} amis sélectionnés",
    saveButton: "✨ Enregistrer",
    createButtonEmoji: "✨ Créer l'événement",
    deleteEventConfirmation:
      "Annuler définitivement cette sortie ? Elle disparaîtra de la liste et le groupe de discussion sera supprimé.",
    eventTitleInput: "Titre de l'événement",
    locationExample: "Ex: Parc Monceau",
    descriptionPlaceholder:
      "Ajoutez des détails, le déroulé, le matériel à prévoir...",

    // LoginPage
    loginTitle: "Happy, let's GO !",
    loginSignIn: "Se connecter",
    loginSignUp: "Créer un compte",
    loginEmailOrId: "Identifiant ou email",
    loginEmail: "Email",
    loginEmailInvalid: "Adresse email invalide (ex. vous@exemple.com).",
    loginEmailFormatHint: "Format email obligatoire (ex. vous@exemple.com).",
    loginPassword: "Mot de passe",
    loginForgotPassword: "Mot de passe oublié ?",
    loginForgotTitle: "Mot de passe oublié",
    loginForgotHint:
      "Entrez votre email. Si un compte existe, vous recevrez un lien pour choisir un nouveau mot de passe (valide 1 h).",
    loginForgotSubmit: "Envoyer le lien",
    loginForgotBack: "Retour à la connexion",
    loginResetTitle: "Nouveau mot de passe",
    loginResetHint: "Choisissez un mot de passe d'au moins 6 caractères.",
    loginResetPassword: "Nouveau mot de passe",
    loginResetConfirm: "Confirmer le mot de passe",
    loginResetSubmit: "Enregistrer et se connecter",
    loginResetMismatch: "Les mots de passe ne correspondent pas.",
    loginDisplayName: "Nom d'affichage",
    loginDisplayNameRequired: "Le nom d'affichage est obligatoire.",
    loginPasswordTooShort: "Le mot de passe doit contenir au moins 6 caractères.",
    loginAge: "Âge",
    loginAgeHint: "Obligatoire — plus de 16 ans",
    loginAgeInvalid: "L'âge est obligatoire et doit être supérieur à 16 ans.",
    loginBioOptional: "Bio (optionnel)",
    loginProAccount: "Compte professionnel",
    loginProCompleteInProfile: "À compléter dans votre profil",
    loginCaptchaLabel: "Je ne suis pas un robot",
    loginCaptchaPlaceholder: "Votre réponse",
    loginCaptchaInvalid: "Réponse incorrecte — réessayez le calcul.",
    loginLockout:
      "Trop de tentatives échouées. Réessayez dans {time}.",
    loginSignupIncomplete: "Pour activer le bouton, complétez :",
    loginLoading: "Chargement...",
    loginNoAccount: "Pas de compte ?",
    loginHasAccount: "Déjà un compte ?",
    loginDemoHint: "Pour tester :",
    loginDemoLabel: "Démo :",
    loginVerifyInProgress: "Vérification de votre email…",
    loginPleaseWait: "Patientez un instant.",
    loginVerifyTitle: "Vérifiez votre email",
    loginVerifySentPrefix: "Un email a été envoyé à",
    loginVerifySentSuffix:
      "Cliquez sur le lien pour activer votre compte, puis connectez-vous.",
    loginResendEmail: "Renvoyer l'email",
    loginResending: "Envoi…",
    loginBackToSignIn: "Retour à la connexion",
    loginGenericError: "Une erreur est survenue",
    loginPlaceholderEmail: "votre@email.com",
    loginPlaceholderId: "em@il.com",
    loginPlaceholderDisplayName: "Votre nom",
    loginPlaceholderAge: "Votre âge",
    loginPlaceholderBio: "Dites-nous un peu sur vous...",

    // Public landing
    landingNavHome: "Accueil",
    landingNavDiscover: "Découvrir",
    landingNavHow: "Comment ça marche",
    landingNavPros: "Professionnels",
    landingNavJoin: "Rejoindre",
    landingHeroBadge: "Communauté bien-être & partage",
    landingHeroTitle: "Bienvenue sur Happy Let's Go",
    landingHeroLead:
      "Une communauté bienveillante de partage et d'échange autour de la spiritualité, du yoga, du bien-être, de la formation, de la danse, des sorties, de la nature, des thérapies, des constellations et bien plus encore.",
    landingHeroSub:
      "Rejoignez des sorties près de chez vous, créez vos événements et rencontrez des professionnels du bien-être pour tisser des liens authentiques.",
    landingCtaJoin: "Créer mon compte",
    landingCtaDiscover: "Explorer les univers",
    landingEyebrowCategories: "Nos univers",
    landingCategoriesTitle: "Douze façons de se retrouver",
    landingCategoriesLead:
      "Chaque sortie sur Happy Let's Go s'inspire d'un univers : retrouvez celui qui vous correspond ou osez en explorer de nouveaux.",
    landingEyebrowHow: "Simple & humain",
    landingHowTitle: "Comment ça marche ?",
    landingHowLead: "Trois étapes pour passer de la curiosité à la rencontre.",
    landingHow1Title: "Créez votre profil",
    landingHow1Desc:
      "Inscrivez-vous gratuitement, présentez-vous en quelques mots et indiquez ce qui vous attire.",
    landingHow2Title: "Découvrez des sorties",
    landingHow2Desc:
      "Parcourez les événements près de chez vous, filtrez par thème et inscrivez-vous en un clic.",
    landingHow3Title: "Rencontrez & partagez",
    landingHow3Desc:
      "Échangez dans le chat de groupe, gardez le contact et créez vos propres sorties.",
    landingEyebrowPros: "Espace pro",
    landingProsTitle: "Vous êtes professionnel du bien-être ?",
    landingProsLead:
      "Yoga, thérapie, formation, danse, naturopathie… Mettez en avant votre activité auprès d'une communauté déjà engagée.",
    landingProBullet1: "Fiche professionnelle visible dans l'annuaire",
    landingProBullet2: "Organisez vos ateliers et sorties",
    landingProBullet3: "Messagerie et notifications en temps réel",
    landingProBullet4: "Badge Pro et visibilité renforcée",
    landingProsCta: "Ouvrir un compte pro",
    landingValuesTitle: "Des rencontres près de chez vous",
    landingValuesText:
      "Happy Let's Go relie les personnes qui veulent prendre soin d'elles et celles qui les accompagnent — en présentiel, dans la bienveillance et le respect.",
    landingCtaTitle: "Prêt·e à nous rejoindre ?",
    landingCtaText:
      "L'inscription est gratuite. Créez votre compte et découvrez la prochaine sortie qui vous ressemble.",
    landingCtaButton: "Commencer maintenant",
    landingFooterTagline: "Partage, bien-être et liens authentiques.",
    landingCat_bien_etre_label: "Bien-être",
    landingCat_bien_etre_desc: "Yoga, méditation, relaxation et rituels pour retrouver l'équilibre.",
    landingCat_forme_label: "Forme",
    landingCat_forme_desc: "Sport doux, renforcement, marche active et énergie du corps.",
    landingCat_dev_personnel_label: "Développement personnel",
    landingCat_dev_personnel_desc: "Ateliers, cercles de parole et chemins de conscience.",
    landingCat_formation_label: "Formation",
    landingCat_formation_desc: "Cours, stages et transmissions pour apprendre et progresser.",
    landingCat_therapy_label: "Thérapie",
    landingCat_therapy_desc: "Accompagnements, soins et approches thérapeutiques bienveillantes.",
    landingCat_nature_label: "Nature",
    landingCat_nature_desc: "Sorties en plein air, reconnexion et calme des espaces verts.",
    landingCat_partage_label: "Partage",
    landingCat_partage_desc: "Moments conviviaux, échanges et lien social authentique.",
    landingCat_musique_label: "Musique",
    landingCat_musique_desc: "Cercles musicaux, chant, vibrations et créativité sonore.",
    landingCat_danse_label: "Danse",
    landingCat_danse_desc: "Danse libre, mouvement intuitif et expression du corps.",
    landingCat_sante_naturelle_label: "Santé naturelle",
    landingCat_sante_naturelle_desc: "Naturopathie, plantes, nutrition et approches holistiques.",
    landingCat_evenement_label: "Sorties & événements",
    landingCat_evenement_desc: "Balades, rencontres thématiques et sorties organisées.",
    landingCat_autres_label: "Autres univers",
    landingCat_autres_desc: "Constellations, arts, spiritualités et expériences singulières.",

    // EventDetailPage
    returnAriaLabel: "Retour",
    shareAriaLabel: "Partager",
    removeFavoriteAriaLabel: "Retirer des favoris",
    addFavoriteAriaLabel: "Ajouter aux favoris",
    reportEventAriaLabel: "Signaler cette sortie",
    participantsCountAriaLabel: "Inscrits",
    viewMyProfile: "Voir mon profil",
    viewProfileAriaLabel: "Voir le profil",
    inviteFriends: "Inviter des amis à cette sortie",
    openChat: "Ouvrir la discussion",
    aboutActivity: "À propos de l'activité",
    waitlistSection: "Liste d'attente",
    inviteFriendsSection: "Inviter des amis",
    comeNumerously: "Venez nombreux...",
    awaitingValidation: "En attente de validation",
    capacityFull: "Capacité complète",
    maxCapacityReachedWaitlist:
      "La capacité maximale est atteinte — ces profils sont en liste d'attente.",

    // OtherProfilePage
    reportProfile: "Signaler ce profil",

    // ReportModal
    reportCancel: "Annuler",
    reportPlaceholder:
      "Exemple : contenu offensant, arnaque, incitation à la haine…",

    // QuestionnaireModal topics/badges
    work: "Travail",
    money: "Argent",
    help: "Aider",
    health: "Santé",
    fatigue: "Fatigue",
    pain: "Douleur",
    happiness: "Bonheur",
    family: "Famille",
    love: "Amour",
    sports: "Sport",
    stress: "Stress",
    calm: "Calme",
    nature: "Nature",
    creativity: "Créativité",
    sex: "Sexualité",

    // QuestionnaireModal questions
    moodQuestion: "Quelle est votre humeur ?",
    mindQuestion: "Qu'est-ce qui occupe votre esprit ?",
    noteQuestion: "Votre pensée du jour",
    moodSubtitle: "Choisissez l'emoji qui vous ressemble le plus aujourd'hui.",
    mindSubtitle: "Sélectionnez un thème qui a marqué votre journée.",
    noteSubtitle: "C'est privé, juste une note pour vous plus tard.",
    questionnaireRecentNotes: "Vos notes récentes",
    questionnaireLastWeekMood: "Votre état d'esprit la semaine dernière",
    questionnaireLastWeekTag: "Semaine dernière",
    incomingFriendRequests: "Demandes reçues",
    pendingFriendRequests: "Demandes envoyées",
    acceptFriendRequest: "Accepter",
    rejectFriendRequest: "Refuser",
    friendRequestAccepted: "Demande acceptée.",
    friendRequestDeclined: "Demande refusée.",
    notifFriendRequestTitle: "Demande d'ami",
    notifFriendRequestBody: "{name} souhaite vous ajouter en ami.",
    notifFriendRequestAcceptedTitle: "Demande acceptée",
    notifFriendRequestAcceptedBody: "{name} a accepté votre demande d'ami.",
    notifFriendRequestRejectedTitle: "Demande refusée",
    notifFriendRequestRejectedBody: "{name} a refusé votre demande d'ami.",
    notifEventInviteReceivedTitle: "Invitation à un évènement",
    notifEventInviteReceivedBody: "{name} vous invite à « {event} ».",

    // ProfilePage organization
    youOrganize: "Vous organisez",

    // EventsPage placeholders
    locationInput: "Lieu",

    // Aria label templates (will be used with template literals)
    unreadCount: "non lus",
    openEvent: "Ouvrir la sortie",
    viewProfileOf: "Voir le profil de",

    // Additional needed translations
    themePlaceholder: "Thème / tag…",
    photoEditBadge: "Modifier",
    dateLabel: "Date",
    timeLabel: "Heure",
    locationLabel: "Lieu",
    maxLabel: "Max",
    aboutTitle: "À PROPOS",
    optionsTitle: "OPTIONS",
    hideAddressLabel: "Masquer l'adresse",
    hideAddressHint: "Visible uniquement par les inscrits",
    manualApprovalLabel: "Validation manuelle",
    manualApprovalHint: "Valider chaque inscription",
    betaEventLabel: "Sortie bêta",
    betaEventHint: "Marquer comme pilote (admin)",
    cancelEventHint:
      "Action définitive: la sortie et son groupe de chat sont supprimés.",
    waitlistPendingMessage: "Inscriptions soumises à validation...",
    nonMemberWarning:
      "Pour voir les messages de cette sortie, vous devez en faire partie.",
    membersLabel: "MEMBRES",
    addMemberButton: "+ un membre",
    inviteFriendsTitle: "Inviter un ami",
    testMessageButton: "Simuler un message entrant (test)",
    coverThemeSectionIntro: "Ou une image par thème",
    leaveGroupConfirm: "Voulez-vous vraiment quitter ce groupe ?",
    leaveConversationConfirm:
      "Voulez-vous vraiment quitter cette conversation ?",

    // EventDetailPage - Additional missing strings
    backButton: "Retour",
    shareButton: "Partager",
    removeFavoriteButton: "Retirer des favoris",
    addFavoriteButton: "Ajouter aux favoris",
    reportButton: "Signaler cette sortie",
    defaultActivity: "Activité",
    proposedByPrefix: "Proposé par",
    locationPlaceholderLabel: "Paris, France",
    waitlistTitle: "Liste d'attente",
    waitlistAwaitingValidation:
      "Inscriptions soumises à validation par l'organisateur avant confirmation.",
    waitlistManualApproval: "Demandes en attente de validation.",
    waitlistOverflowOnly:
      "La capacité maximale est atteinte — ces profils sont en liste d'attente.",
    waitlistBothPendingAndOverflow:
      "Certaines demandes attendent la validation de l'organisateur ; d'autres suivent alors que la capacité est atteinte.",
    awaitingValidationTag: "En attente de validation",
    capacityFullTag: "Capacité complète",
    viewMyProfileButton: "Voir mon profil",
    viewProfileButton: "Voir le profil de",
    inviteFriendsToEventButton: "Inviter des amis à cette sortie",
    openChatButton: "Ouvrir la discussion",
    pricePerPerson: "par personne",
    editEventButton: "Modifier la sortie",
    unregisterButton: "Se désinscrire",
    joinEventButton: "Participer",
    payAndJoinEventButton: "Payer le Billet de confiance et participer",
    eventCheckoutTitle: "Billet de confiance",
    eventPaymentTotal: "Total",
    eventTipPaymentLabel: "Billet de confiance",
    eventTrustTicketCheckoutHint:
      "Frais de réservation pour l'application Happy Let's Go.",
    eventPaymentSuccess:
      "Billet de confiance validé — vous êtes inscrit·e à la sortie.",
    joinEventKarmaCost: "−{cost} karma",
    joinEventKarmaReward: "+{reward} karma si présence validée",
    completeEventButton: "Complet",
    pastEventLabel: "Sortie passée",
    mustJoinToViewMessages:
      "Pour voir les messages de cette sortie, vous devez en faire partie.",
    inviteSheetTitle: "Inviter des amis",
    adminInviteSheetTitle: "Inviter des personnes",
    adminInviteSection: "Inviter des personnes",
    adminInviteHint:
      "Choisissez qui inviter — une notification sera envoyée pour rejoindre la sortie.",
    adminInviteSearchPlaceholder: "Rechercher un nom…",
    adminInviteAll: "Inviter tout le monde ({{n}})",
    adminInviteAllConfirm:
      "Envoyer une invitation à {{n}} personnes ? Chacune recevra une notification.",
    adminInviteEmpty: "Tout le monde est déjà invité ou dans le groupe.",
    adminInviteSearchNoResults: "Aucune personne ne correspond à votre recherche.",
    inviteAction: "Inviter",
    inviteHint:
      "Vos amis recevront une notification pour rejoindre cette sortie.",
    noMoreSlotsHint: "Plus de places disponibles",

    // ChatPage - Missing strings
    chatMessages: "Messages",
    chatSuggestions: "Suggestions",
    chatSuggestionsLoadingMore: "Chargement des profils suivants…",
    noSuggestions: "Aucune suggestion pour le moment.",
    chatVisits: "Visites",
    searchButton: "Rechercher",
    chatUserSearchPlaceholder: "Rechercher un utilisateur ou un groupe…",
    chatUserSearchHint: "Saisissez un nom, une ville, un pseudo ou le titre d'un groupe.",
    chatUserSearchNoResults: "Aucun utilisateur ni groupe ne correspond à votre recherche.",
    chatSearchGroupTag: "Groupe",
    chatUserSearchPremiumOnly: "La recherche est réservée aux comptes Premium ou Pro.",
    premiumFeature: "Fonctionnalité Premium",
    visitsPlaceholder: "personnes ont visité votre profil",
    friendRequestBadge: "Demande d'ami",
    visitMultiplier: "×",
    friendLabel: "Ami·e",
    requestRejected: "Demande d'ami refusée",
    requestSent: "Demande d'ami envoyée",
    sendFriendRequest: "Envoyer une demande d'ami",
    rejectedRequest: "Refusée",
    sentRequest: "Envoyée",
    addFriendButton: "Ajouter",
    friendRequestDailyLimit: "Une demande d'ami par jour",

    // ChatRoomPage - Missing strings
    membersCount: "membres",
    directMessageLabel: "Message direct",
    viewEventButton: "Voir",
    messageInputHint: "Écrivez un message…",
    chatThreadClosedHint:
      "Cette discussion est fermée (plus de 7 jours). Consultation seule.",
    chatThreadClosedPlaceholder: "Discussion fermée",

    // ChatSettingsPage - Missing strings
    chatSettingsTitle: "PARAMÈTRES DE DISCUSSION",
    muteSoundsLabel: "Couper les sons",
    blockNotificationsLabel: "Bloquer les notifications",
    simpleTestMessage: "Simuler un message entrant (test)",
    membersTitle: "MEMBRES",
    addMemberHint: "+ un membre",
    inviteMembersTitle: "Inviter un ami",
    noMoreFriendsToAdd: "Tous vos amis sont déjà dans le groupe.",
    viewMyProfileLabel: "Voir mon profil",
    viewProfileLabel: "Voir le profil de",
    removeMemberConfirm: "Voulez-vous vraiment retirer ce membre du groupe ?",
    leaveGroupLabel: "Sortir du groupe",
    leaveConversationLabel: "Quitter la conversation",
    leaveGroupConfirmMessage: "Voulez-vous vraiment quitter ce groupe ?",
    leaveConversationConfirmMessage:
      "Voulez-vous vraiment quitter cette conversation ?",

    // CreateEventPage - Missing strings
    createEventTitle: "Nouvelle sortie",
    editEventTitle: "Modifier la sortie",
    closeButton: "Fermer",
    createButton: "Créer",
    saveBadgeButton: "✨ Enregistrer",
    createBadgeButton: "✨ Créer l'événement",
    coverThemeIntro: "Ou une image par thème",
    dateHint: "Date",
    timeHint: "Heure",
    locationFieldLabel: "Lieu",
    locationFieldExample: "Ex: Parc Monceau",
    increaseMaxButton: "Augmenter le maximum",
    decreaseMaxButton: "Diminuer le maximum",
    descriptionFieldHint:
      "Ajoutez des détails, le déroulé, le matériel à prévoir...",
    cancelButton: "Annuler",
    closeEventButton: "Fermer",
    cancelSortieConfirm:
      "Annuler définitivement cette sortie ? Elle disparaîtra de la liste et le groupe de discussion sera supprimé.",

    // OtherProfilePage - Missing strings
    otherProfileBio: "Pas de bio pour le moment.",
    profileVerifiedBadge: "Profil vérifié",
    memberSincePrefix: "Membre depuis",
    statsReliability: "Fiabilité",
    statsEvents: "Événements",
    statsFriends: "Amis",
    badgesSection: "Badges",
    favoritesAndHostedTab: "Favoris & créées",
    friendsTab: "Amis",
    pastTab: "Passés",
    noUpcomingForProfile:
      "Aucune sortie à venir (favori ou organisateur) liée à ce profil. Les dates passées sont dans « Passés ».",
    noNetworkPreview: "Aucun aperçu réseau pour ce profil.",
    noPastForProfile: "Aucune sortie passée liée à ce profil.",
    viewButton: "Voir",
    organizerLabel: "Organisateur",
    eventsInCommon: "communs",
  },
  en: {
    // ProfilePage
    settings: "Settings",
    loading: "Loading...",
    premium: "Premium",
    premiumSub: "All features unlocked",
    premiumPrice: "€9.99/mo",
    proPrice: "€19.99/mo",
    subscribe: "Subscribe",
    subscriptionActive: "Active",
    subscriptionUntil: "Valid until",
    subscriptionUntilUnknown: "Active subscription (end date not set)",
    cancelSubscription: "Cancel subscription",
    cancelSubscriptionConfirm: "Cancel this subscription? Premium/Pro access will be disabled immediately.",
    subscriptionCancelledPremium: "Premium subscription cancelled.",
    subscriptionCancelledPro: "Pro subscription cancelled.",
    subscriptionTotal: "Total",
    subscriptionDuration: "Subscription length",
    subscriptionMonthsCount: "{{n}} months",
    subscriptionPerMonth: "/ month",
    payWithCardBeta: "Free during beta 😉",
    cardholderName: "Cardholder name",
    cardholderPlaceholder: "Name on card",
    cardNumber: "Card number",
    cardExpiry: "Expiry",
    cardCvc: "CVC",
    payWithCard: "Pay with card",
    paymentProcessing: "Processing payment…",
    paymentSecureNote: "Simulated payment — ready for Stripe or PayPlug integration.",
    paymentFailed: "Payment failed. Please try again.",
    paymentSuccessPremium: "Premium activated. Thank you!",
    paymentSuccessPro: "Pro account activated. Thank you!",
    cardholderRequired: "Enter the cardholder name.",
    cardNumberInvalid: "Invalid card number.",
    cardExpiryInvalid: "Invalid expiry date.",
    cardCvcInvalid: "Invalid CVC code.",
    adminMode: "Admin Mode",
    adminModeSub: "Access to moderation tools",
    darkMode: "Dark mode",
    darkModeSub: "Dark interface (default)",
    adminInfoTab: "Info",
    adminInfoSkipEmailLabel: "Sign up without email verification",
    adminInfoSkipEmailSub:
      "Auto-validates new accounts (useful when email delivery is not configured)",
    adminInfoSplashLabel: "Startup splash screen",
    adminInfoSplashSub: "Show the « Happy, let's GO » screen when the app opens",
    adminInfoSplashImageLabel: "Splash image",
    adminInfoSplashImageHint:
      "Shown to everyone on startup. URL is synced via app_config.",
    adminInfoSplashImageUpload: "Change image",
    adminInfoSplashImageUploading: "Uploading…",
    adminInfoSplashImageReset: "Default image",
    adminInfoSplashImageSaved: "Splash image saved",
    adminInfoModalLabel: "Information modal",
    adminInfoModalSub: "Show a message to all signed-in users",
    adminInfoModalDismissLabel: "Allow dismissal",
    adminInfoModalDismissSub: "Show a close button on the modal",
    adminInfoMessageLabel: "Message for everyone",
    adminInfoMessagePlaceholder:
      "Updates, important events, maintenance…",
    adminInfoMessageHint:
      "Write your text, then publish to send it to all users.",
    adminInfoPublish: "Publish to everyone",
    adminInfoPublished: "Message published",
    adminInfoForceReloadLabel: "Force app update",
    adminInfoForceReloadSub:
      "On publish, ask everyone to reload the application",
    announcementModalTitle: "Information",
    appForceReloadTitle: "Update required",
    appForceReloadBody:
      "A new version of the app is available. Reload to stay up to date.",
    appForceReloadAction: "Reload application",
    adminEditProfile: "Edit profile (admin)",
    adminDeleteProfile: "Delete user",
    adminDeleteProfileConfirm:
      "Permanently delete this user? They will be removed from suggestions, visits, and private chats.",
    adminDeleteEvent: "Delete event",
    adminDeleteEventConfirm:
      "Permanently delete this event? It and its group chat will be removed.",
    adminDeleteConversation: "Delete conversation",
    adminDeleteConversationConfirm:
      "Permanently delete this conversation and all its messages?",
    professional: "Professional account",
    professionalSub: "Organizer of activities and events PRO",
    calendarTab: "Calendar",
    previousMonth: "Previous month",
    nextMonth: "Next month",
    noEventsOnSelectedDay: "No events you created on this day.",
    remindButton: "Remind",
    reminderSent: "Reminder sent",
    reminderRead: "Read",
    reminderUnread: "Sent",
    remindAllButton: "Remind all",
    myProAgenda: "My Professional Agenda",
    language: "Language",
    clearCache: "Clear cache",
    clearCacheButton: "Clear",
    clearCacheConfirm: "Clear local cache and reload data from the server?",
    clearCacheSuccess: "Cache cleared. Data reloaded.",
    clearCacheError: "Could not clear cache.",
    betaFeatures: "Enable Beta Features",
    howToInstall: "How to install",
    howToInstallTitle: "Install the app",
    howToInstallIos: "On iPhone (Safari)",
    howToInstallAndroid: "On Android",
    infoButton: "Info",
    logout: "Logout",
    deleteAccount: "Delete my account",

    // Profile tabs
    favoritesCreated: "My outings",
    friends: "Friends",
    reports: "Reports",
    history: "Past",
    notifications: "Notifications",

    // Profile sections
    badges: "Badges",
    badgeAddPlaceholder: "Nouveau badge…",
    badgeAddAria: "Ajouter un badge",
    badgeRemoveAria: "Retirer le badge",
    badgeAdminHint: "Admin mode: tap × to remove, + to add a badge.",
    badgeCatalogTitle: "Badge catalog",
    badgeQuickAddTitle: "Quick add",
    badgeCatalogAdd: "New badge",
    badgeCatalogAddPlaceholder: "Badge name…",
    badgeEditAria: "Edit badge",
    badgeRemoveCatalogAria: "Remove from catalog",
    reliability: "Reliability",
    upcoming: "Upcoming",
    upcomingOutings: "Upcoming outings",
    noShows: "No-shows",

    // Bio card
    memberSince: "Member since 2024",
    editProfile: "Edit profile",
    cancel: "Cancel",
    save: "Save",
    verified: "Verified profile",
    adminProVerifiedTitle: "Pro verification",
    adminProVerifiedSub: "Manually enabled by an admin — badge shown in the Pro directory.",

    // Empty states
    noFavoritesUpcoming:
      'No upcoming outings: favorites, events you organize, or events you joined. Past dates are in "Past".',
    noFriends: "No friends yet.",
    noReports: "No reports to handle.",
    noNotifications: "No notifications.",
    notifParticipantJoinedTitle: "New participant",
    notifParticipantJoinedBody: "{name} joined « {title} ».",
    notifParticipantLeftTitle: "Unregistered",
    notifParticipantLeftBody: "{name} left « {title} ».",
    notifWaitlistJoinedTitle: "Waitlist",
    notifWaitlistJoinedBody: "{name} joined the waitlist for « {title} ».",
    notifWaitlistLeftTitle: "Waitlist",
    notifWaitlistLeftBody: "{name} left the waitlist for « {title} ».",
    notifWaitlistAcceptedTitle: "Request accepted",
    notifWaitlistAcceptedBody: "{name} accepted your request for « {title} ».",
    notifWaitlistRejectedTitle: "Request declined",
    notifWaitlistRejectedBody: "{name} declined your request for « {title} ».",
    noPastActivity: "No past activity.",

    // Reports
    suspiciousLogin: "Suspicious login",
    profile: "Profile",
    chat: "Messages",
    navChat: "Chat",
    navSortie: "Events",
    pro: "Pro",
    professionalsTitle: "Professionals",
    proSearchPlaceholder: "Search name, city, description…",
    proFilterLabel: "Filter by profession",
    proJobLabel: "Profession",
    proJobPlaceholder: "E.g. Sophrologist, wellness massage, mobile hairdressing…",
    proCategoryTypeLabel: "Profession type",
    proCategoryTypeHint: "Category shown in the directory and used for filtering.",
    proCategoryLabel: "Profession",
    proFilterAll: "All",
    proNoResults: "No professionals match your search.",
    proViewModeLabel: "View mode",
    proViewList: "List",
    proViewMap: "Map",
    proClientsLabel: "Clients",
    proContactButton: "Contact",
    proContactSoon: "Pro messaging coming soon.",
    proWebsiteLabel: "Website",
    proSocialLabel: "Social media",
    proPhoneLabel: "Phone",
    proWebsitePlaceholder: "https://www.my-site.com",
    proSocialPlaceholder: "https://instagram.com/myaccount",
    proPhonePlaceholder: "+33 6 12 34 56 78",
    proAddressLabel: "Practice address",
    proAddressPlaceholder: "1 Place du Capitole, 31000 Toulouse",
    proAddressHint: "Number, street, postal code and city (spaces are allowed). Validated on save for the Pro map.",
    proAddressFormatHint: "Address too short. E.g. 1 Place du Capitole, 31000 Toulouse",
    proAddressGeocoding: "Validating…",
    proAddressInvalid: "Address not found. Check street, postal code and city.",
    proAddressValidated: "Address validated — shown on the Pro map.",
    event: "Event",
    dismiss: "Dismiss",
    hideAndNotify: "Hide and notify",

    // Notifications
    invitationSent: "Invitation sent",
    willNotify: "will receive a notification to join",

    // Settings
    photoUploadFailed: "Photo upload failed",
    photoUploading: "Uploading photo…",
    changePhoto: "Change photo",

    // HomePage
    searchActivity: "Search for an activity…",
    topActivities: "Top 5 Activities",
    activitiesSchedule: "Activities Schedule",
    all: "All",
    sport: "Sport",
    outings: "Outings",
    culture: "Culture",

    // EventCard
    beta: "Beta",
    registered: "Registered",
    register: "Register",
    organizer: "Organizer",
    pending: "Pending",
    addFavorite: "Add to favorites",
    removeFavorite: "Remove from favorites",
    you: "You",
    viewing: "Viewing",
    available: "Available",
    full: "Full",

    // EventDetailPage
    confirmUnregister: "Do you really want to unregister from this activity?",
    aboutEvent: "About the activity",
    waitlist: "Waitlist",
    proposedBy: "Proposed by",
    linkCopied: "Link copied to clipboard!",
    karmaShort: "Karma",
    karmaPointsLabel: "{count} karma points",
    validatePresenceAriaLabel: "Validate attendance (+2 karma)",
    rateOrganizerTitle: "How was the outing?",
    rateOrganizerSubtitle: "Your rating affects the organizer's karma.",
    rateOrganizerGood: "Good",
    rateOrganizerBad: "Bad",
    rateOrganizerGoodAria: "Rate organizer positively",
    rateOrganizerBadAria: "Rate organizer negatively",
    rateOrganizerThanks: "Thanks for your feedback!",
    createEventKarmaCost: "−{cost} karma",
    createEventKarmaFree: "Free (Pro)",
    createEventKarmaReward: "+{reward} karma if outing succeeds (≥ half registered)",
    shareLinkTooltip: "Click to copy link",
    participants: "Participants",
    unregister: "Unregister",
    joinWaitlist: "Join waitlist",
    leaveWaitlist: "Leave waitlist",
    approveWaitlistEntry: "Approve request",
    rejectWaitlistEntry: "Reject request",
    share: "Share",
    report: "Report",
    yes: "Yes",
    no: "No",

    // CreateEventPage
    eventTitlePlaceholder: "Enter a title for your event.",
    locationPlaceholder: "Enter a location or meeting point.",
    participantsError: "The number of participants must be between 1 and 100.",
    imageUploadFailed: "Image upload failed.",
    createEvent: "Create Event",
    title: "Title",
    location: "Location",
    date: "Date",
    time: "Time",
    maxParticipants: "Max Participants",
    description: "Description",
    eventPriceLabel: "Price",
    eventPriceFree: "Free",
    eventPricePaid: "Paid",
    eventPricePlaceholder: "Price per person",
    eventJoinTipEnable: "Offer a Trust Ticket",
    eventJoinTipRequired: "Trust Ticket required (paid event)",
    eventJoinTipHint:
      "App reservation fee: participants pay this amount to confirm their registration for the event.",
    eventJoinTipAmountLabel: "Trust Ticket amount",

    // ChatPage
    newGroup: "New Group",
    addGroup: "+ Group",
    createGroupNamePrompt: "Group name:",
    createGroupNameRequired: "Please enter a group name.",
    noChatRooms: "No conversations yet.",
    favoriteChatConversations: "Favorite Conversations",

    // ChatRoomPage
    members: "members",
    directMessage: "Direct Message",
    view: "View",
    messageInputPlaceholder: "Write a message…",
    sendMessage: "Send",

    // ChatSettingsPage
    chatSettings: "CHAT SETTINGS",
    muteNotifications: "Mute notifications",
    blockNotifications: "Block notifications",
    leaveGroup: "Leave group",
    deleteGroup: "Delete group",
    allFriendsInGroup: "All your friends are already in this group.",

    // QuestionnaireModal
    howAreYou: "How are you?",
    step: "STEP",
    of: "OF",
    step1: "STEP 1 OF 3",
    step2: "STEP 2 OF 3",
    step3: "STEP 3 OF 3",
    continue: "Continue",
    skip: "Skip",
    questionPlaceholder: "Write something here...",
    badges_punctual: "Punctual",
    badges_organizer: "Organizer",
    badges_friendly: "Friendly",
    badges_explorer: "Explorer",

    // ReportModal
    reportExplanation: "Describe what seems inappropriate or dangerous to you.",
    explanation: "Explanation",
    required: "(required)",
    exampleOffensive: "Example: offensive content…",
    describeIssue: "Please describe the issue.",
    send: "Send",

    // OtherProfilePage
    noBio: "No bio yet.",
    events: "Events",
    pioneer: "Pioneer",

    // Form inputs
    name: "Name",
    age: "Age",
    cityPlaceholder: "City (e.g. Toulouse)",
    bio: "Bio...",
    placeholder: "Bio...",

    // Navigation
    back: "Back",
    close: "Close",

    // Aria labels & buttons
    settingsAriaLabel: "Settings",
    notificationSoundEnable: "Enable notification sound",
    notificationSoundDisable: "Disable notification sound",
    searchAriaLabel: "Search",
    previousWeek: "Previous week",
    nextWeek: "Next week",
    calendarAriaLabel: "Calendar",
    clearSearch: "Clear",
    validateSearch: "Validate",
    backToTextSearch: "Back to text search",
    dateFilterAriaLabel: "Filter by date (from this date)",
    locationFilterAriaLabel: "Filter by location",
    themeFilterAriaLabel: "Filter by theme (same list as covers)",
    filtersAriaLabel: "Filters: date, location, tag",
    createEventAriaLabel: "Create an event",
    topWeeklyEvents: "Top 5 of the week",
    noEventsThisWeek: "No events this week.",
    noSearchResults: "No activities match your search.",
    eventsSearchLoadingMore: "Loading more events…",
    searchEvents: "Search",

    // CreateEventPage aria-labels
    themeCoversSuggestions: "Suggested theme covers",
    addCoverPhoto: "Add a cover photo",
    increaseMax: "Increase maximum",
    decreaseMax: "Decrease maximum",
    privateEventToggle: "Private event",
    privateEventHint:
      "Hidden from the public agenda; visible to you and registered members",
    aboutSectionTitle: "ABOUT",
    optionsSectionTitle: "OPTIONS",
    createEventParticipantsRangeAlert:
      "The number of participants must be between {min} and {max}.",
    createEventErrorTitle: "Enter a title for your event.",
    createEventErrorLocation: "Enter a location or meeting point.",
    createEventErrorPrice: "Enter a valid price (whole number greater than 0) or choose Free.",
    createEventErrorNotFound: "This event could not be found.",
    createEventErrorGeneric:
      "Could not create the event. Check the fields and try again.",
    createEventKarmaEnrollmentHint:
      "Bonus only if at least half the spots are filled",
    deleteEvent: "Cancel the event",
    editEvent: "Edit the event",
    newEvent: "New event",
    saveChanges: "Save changes",
    createEventButton: "Create event",
    createEventInviteAll: "All",
    createEventInviteNone: "None",
    createEventInviteSelectedOne: "1 friend selected",
    createEventInviteSelectedMany: "{{n}} friends selected",
    saveButton: "✨ Save",
    createButtonEmoji: "✨ Create event",
    deleteEventConfirmation:
      "Permanently cancel this event? It will disappear from the list and the discussion group will be deleted.",
    eventTitleInput: "Event title",
    locationExample: "Ex: Central Park",
    descriptionPlaceholder: "Add details, schedule, materials to prepare...",

    // LoginPage
    loginTitle: "Happy, let's GO !",
    loginSignIn: "Sign in",
    loginSignUp: "Create an account",
    loginEmailOrId: "Username or email",
    loginEmail: "Email",
    loginEmailInvalid: "Invalid email address (e.g. you@example.com).",
    loginEmailFormatHint: "Email format required (e.g. you@example.com).",
    loginPassword: "Password",
    loginForgotPassword: "Forgot password?",
    loginForgotTitle: "Forgot password",
    loginForgotHint:
      "Enter your email. If an account exists, you'll receive a link to set a new password (valid for 1 hour).",
    loginForgotSubmit: "Send reset link",
    loginForgotBack: "Back to sign in",
    loginResetTitle: "New password",
    loginResetHint: "Choose a password with at least 6 characters.",
    loginResetPassword: "New password",
    loginResetConfirm: "Confirm password",
    loginResetSubmit: "Save and sign in",
    loginResetMismatch: "Passwords do not match.",
    loginDisplayName: "Display name",
    loginDisplayNameRequired: "Display name is required.",
    loginPasswordTooShort: "Password must be at least 6 characters.",
    loginAge: "Age",
    loginAgeHint: "Required — must be over 16",
    loginAgeInvalid: "Age is required and must be over 16.",
    loginBioOptional: "Bio (optional)",
    loginProAccount: "Professional account",
    loginProCompleteInProfile: "Complete in your profile",
    loginCaptchaLabel: "I'm not a robot",
    loginCaptchaPlaceholder: "Your answer",
    loginCaptchaInvalid: "Wrong answer — try the calculation again.",
    loginLockout: "Too many failed attempts. Try again in {time}.",
    loginSignupIncomplete: "To enable the button, complete:",
    loginLoading: "Loading...",
    loginNoAccount: "Don't have an account?",
    loginHasAccount: "Already have an account?",
    loginDemoHint: "To try the app:",
    loginDemoLabel: "Demo:",
    loginVerifyInProgress: "Verifying your email…",
    loginPleaseWait: "Please wait a moment.",
    loginVerifyTitle: "Verify your email",
    loginVerifySentPrefix: "An email was sent to",
    loginVerifySentSuffix:
      "Click the link to activate your account, then sign in.",
    loginResendEmail: "Resend email",
    loginResending: "Sending…",
    loginBackToSignIn: "Back to sign in",
    loginGenericError: "Something went wrong",
    loginPlaceholderEmail: "you@email.com",
    loginPlaceholderId: "em@il.com",
    loginPlaceholderDisplayName: "Your name",
    loginPlaceholderAge: "Your age",
    loginPlaceholderBio: "Tell us a bit about yourself...",

    // Public landing
    landingNavHome: "Home",
    landingNavDiscover: "Discover",
    landingNavHow: "How it works",
    landingNavPros: "Professionals",
    landingNavJoin: "Join",
    landingHeroBadge: "Wellness & sharing community",
    landingHeroTitle: "Welcome to Happy Let's Go",
    landingHeroLead:
      "A caring community for sharing around spirituality, yoga, wellness, training, dance, outings, nature, therapies, constellations and much more.",
    landingHeroSub:
      "Join events near you, create your own, and meet wellness professionals to build authentic connections.",
    landingCtaJoin: "Create my account",
    landingCtaDiscover: "Explore themes",
    landingEyebrowCategories: "Our worlds",
    landingCategoriesTitle: "Twelve ways to connect",
    landingCategoriesLead:
      "Every outing on Happy Let's Go belongs to a theme — find yours or explore something new.",
    landingEyebrowHow: "Simple & human",
    landingHowTitle: "How does it work?",
    landingHowLead: "Three steps from curiosity to real-world connection.",
    landingHow1Title: "Create your profile",
    landingHow1Desc:
      "Sign up for free, introduce yourself briefly, and share what draws you in.",
    landingHow2Title: "Discover events",
    landingHow2Desc:
      "Browse outings near you, filter by theme, and join with one tap.",
    landingHow3Title: "Meet & share",
    landingHow3Desc:
      "Chat in the event group, stay in touch, and host your own outings.",
    landingEyebrowPros: "Pro space",
    landingProsTitle: "Are you a wellness professional?",
    landingProsLead:
      "Yoga, therapy, training, dance, naturopathy… Showcase your practice to an engaged community.",
    landingProBullet1: "Professional profile in the directory",
    landingProBullet2: "Host workshops and outings",
    landingProBullet3: "Real-time messaging and notifications",
    landingProBullet4: "Pro badge and stronger visibility",
    landingProsCta: "Open a pro account",
    landingValuesTitle: "Meetings close to home",
    landingValuesText:
      "Happy Let's Go connects people who care for themselves with those who guide them — in person, with kindness and respect.",
    landingCtaTitle: "Ready to join us?",
    landingCtaText:
      "Sign-up is free. Create your account and find the next outing made for you.",
    landingCtaButton: "Get started",
    landingFooterTagline: "Sharing, wellness and authentic bonds.",
    landingCat_bien_etre_label: "Wellness",
    landingCat_bien_etre_desc: "Yoga, meditation, relaxation and rituals to restore balance.",
    landingCat_forme_label: "Fitness",
    landingCat_forme_desc: "Gentle sport, strengthening, active walks and body energy.",
    landingCat_dev_personnel_label: "Personal growth",
    landingCat_dev_personnel_desc: "Workshops, sharing circles and paths of awareness.",
    landingCat_formation_label: "Training",
    landingCat_formation_desc: "Classes, retreats and learning experiences.",
    landingCat_therapy_label: "Therapy",
    landingCat_therapy_desc: "Support, care and compassionate therapeutic approaches.",
    landingCat_nature_label: "Nature",
    landingCat_nature_desc: "Outdoor outings, reconnection and calm green spaces.",
    landingCat_partage_label: "Sharing",
    landingCat_partage_desc: "Friendly moments, exchanges and authentic social bonds.",
    landingCat_musique_label: "Music",
    landingCat_musique_desc: "Music circles, singing, vibrations and sonic creativity.",
    landingCat_danse_label: "Dance",
    landingCat_danse_desc: "Free dance, intuitive movement and body expression.",
    landingCat_sante_naturelle_label: "Natural health",
    landingCat_sante_naturelle_desc: "Naturopathy, plants, nutrition and holistic approaches.",
    landingCat_evenement_label: "Outings & events",
    landingCat_evenement_desc: "Walks, themed meetups and organized outings.",
    landingCat_autres_label: "Other worlds",
    landingCat_autres_desc: "Constellations, arts, spiritual paths and unique experiences.",

    // EventDetailPage
    returnAriaLabel: "Back",
    shareAriaLabel: "Share",
    removeFavoriteAriaLabel: "Remove from favorites",
    addFavoriteAriaLabel: "Add to favorites",
    reportEventAriaLabel: "Report this event",
    participantsCountAriaLabel: "Registered",
    viewMyProfile: "View my profile",
    viewProfileAriaLabel: "View profile",
    inviteFriends: "Invite friends to this event",
    openChat: "Open chat",
    aboutActivity: "About the activity",
    waitlistSection: "Waitlist",
    inviteFriendsSection: "Invite friends",
    comeNumerously: "Come numerous...",
    awaitingValidation: "Awaiting validation",
    capacityFull: "Capacity full",
    maxCapacityReachedWaitlist:
      "Maximum capacity reached — these profiles are on the waitlist.",

    // OtherProfilePage
    reportProfile: "Report this profile",

    // ReportModal
    reportCancel: "Cancel",
    reportPlaceholder: "Example: offensive content, scam, hate speech…",

    // QuestionnaireModal topics/badges
    work: "Work",
    money: "Money",
    help: "Help",
    health: "Health",
    fatigue: "Fatigue",
    pain: "Pain",
    happiness: "Happiness",
    family: "Family",
    love: "Love",
    sports: "Sports",
    stress: "Stress",
    calm: "Calm",
    nature: "Nature",
    creativity: "Creativity",
    sex: "Sexuality",

    // QuestionnaireModal questions
    moodQuestion: "What is your mood?",
    mindQuestion: "What's on your mind?",
    noteQuestion: "Your thought of the day",
    moodSubtitle: "Choose the emoji that best describes you today.",
    mindSubtitle: "Select a theme that marked your day.",
    noteSubtitle: "It's private, just a note for you later.",
    questionnaireRecentNotes: "Your recent notes",
    questionnaireLastWeekMood: "Your mood last week",
    questionnaireLastWeekTag: "Last week",
    incomingFriendRequests: "Incoming requests",
    pendingFriendRequests: "Sent requests",
    acceptFriendRequest: "Accept",
    rejectFriendRequest: "Decline",
    friendRequestAccepted: "Request accepted.",
    friendRequestDeclined: "Request declined.",
    notifFriendRequestTitle: "Friend request",
    notifFriendRequestBody: "{name} wants to add you as a friend.",
    notifFriendRequestAcceptedTitle: "Request accepted",
    notifFriendRequestAcceptedBody: "{name} accepted your friend request.",
    notifFriendRequestRejectedTitle: "Request declined",
    notifFriendRequestRejectedBody: "{name} declined your friend request.",
    notifEventInviteReceivedTitle: "Event invitation",
    notifEventInviteReceivedBody: "{name} invited you to « {event} ».",

    // ProfilePage organization
    youOrganize: "You organize",

    // EventsPage placeholders
    locationInput: "Location",

    // Aria label templates (will be used with template literals)
    unreadCount: "unread",
    openEvent: "Open event",
    viewProfileOf: "View profile of",

    // Additional needed translations
    themePlaceholder: "Theme / tag…",
    photoEditBadge: "Edit",
    dateLabel: "Date",
    timeLabel: "Time",
    locationLabel: "Location",
    maxLabel: "Max",
    aboutTitle: "ABOUT",
    optionsTitle: "OPTIONS",
    hideAddressLabel: "Hide address",
    hideAddressHint: "Visible only to registered members",
    manualApprovalLabel: "Manual validation",
    manualApprovalHint: "Validate each registration",
    betaEventLabel: "Beta event",
    betaEventHint: "Mark as pilot (admin)",
    cancelEventHint:
      "Definitive action: the event and its discussion group will be deleted.",
    waitlistPendingMessage: "Registrations pending validation...",
    nonMemberWarning:
      "To see the messages of this event, you must be part of it.",
    membersLabel: "MEMBERS",
    addMemberButton: "+ Add member",
    inviteFriendsTitle: "Invite a friend",
    testMessageButton: "Simulate incoming message (test)",
    coverThemeSectionIntro: "Or image by theme",
    leaveGroupConfirm: "Do you really want to leave this group?",
    leaveConversationConfirm: "Do you really want to leave this conversation?",

    // EventDetailPage - Additional missing strings
    backButton: "Back",
    shareButton: "Share",
    removeFavoriteButton: "Remove from favorites",
    addFavoriteButton: "Add to favorites",
    reportButton: "Report this event",
    defaultActivity: "Activity",
    proposedByPrefix: "Proposed by",
    locationPlaceholderLabel: "Paris, France",
    waitlistTitle: "Waitlist",
    waitlistAwaitingValidation:
      "Registrations pending organizer validation before confirmation.",
    waitlistManualApproval: "Requests pending validation.",
    waitlistOverflowOnly:
      "Maximum capacity reached — these profiles are on the waitlist.",
    waitlistBothPendingAndOverflow:
      "Some requests await organizer validation; others follow as capacity is reached.",
    awaitingValidationTag: "Awaiting validation",
    capacityFullTag: "Capacity full",
    viewMyProfileButton: "View my profile",
    viewProfileButton: "View profile of",
    inviteFriendsToEventButton: "Invite friends to this event",
    openChatButton: "Open chat",
    pricePerPerson: "per person",
    editEventButton: "Edit event",
    unregisterButton: "Unregister",
    joinEventButton: "Join",
    payAndJoinEventButton: "Pay Trust Ticket and join",
    eventCheckoutTitle: "Trust Ticket",
    eventPaymentTotal: "Total",
    eventTipPaymentLabel: "Trust Ticket",
    eventTrustTicketCheckoutHint:
      "Reservation fee for the Happy Let's Go app.",
    eventPaymentSuccess:
      "Trust Ticket confirmed — you are registered for this event.",
    joinEventKarmaCost: "−{cost} karma",
    joinEventKarmaReward: "+{reward} karma if attendance validated",
    completeEventButton: "Full",
    pastEventLabel: "Past event",
    mustJoinToViewMessages: "You must join this event to view messages.",
    inviteSheetTitle: "Invite friends",
    adminInviteSheetTitle: "Invite people",
    adminInviteSection: "Invite people",
    adminInviteHint:
      "Choose who to invite — they will receive a notification to join the event.",
    adminInviteSearchPlaceholder: "Search by name…",
    adminInviteAll: "Invite everyone ({{n}})",
    adminInviteAllConfirm:
      "Send an invitation to {{n}} people? Each will receive a notification.",
    adminInviteEmpty: "Everyone is already invited or in the group.",
    adminInviteSearchNoResults: "No one matches your search.",
    inviteAction: "Invite",
    inviteHint: "Your friends will receive a notification to join this event.",
    noMoreSlotsHint: "No more spots available",

    // ChatPage - Missing strings
    chatMessages: "Messages",
    chatSuggestions: "Suggestions",
    chatSuggestionsLoadingMore: "Loading more profiles…",
    noSuggestions: "No suggestions at the moment.",
    chatVisits: "Visits",
    searchButton: "Search",
    chatUserSearchPlaceholder: "Search for a user or group…",
    chatUserSearchHint: "Enter a name, city, username, or group title.",
    chatUserSearchNoResults: "No users or groups match your search.",
    chatSearchGroupTag: "Group",
    chatUserSearchPremiumOnly: "Search is available for Premium or Pro accounts.",
    premiumFeature: "Premium Feature",
    visitsPlaceholder: "people visited your profile",
    friendRequestBadge: "Friend request",
    visitMultiplier: "×",
    friendLabel: "Friend",
    requestRejected: "Friend request rejected",
    requestSent: "Friend request sent",
    sendFriendRequest: "Send friend request",
    rejectedRequest: "Rejected",
    sentRequest: "Sent",
    addFriendButton: "Add",
    friendRequestDailyLimit: "One friend request per day",

    // ChatRoomPage - Missing strings
    membersCount: "members",
    directMessageLabel: "Direct message",
    viewEventButton: "View",
    messageInputHint: "Write a message…",
    chatThreadClosedHint:
      "This conversation is closed (over 7 days old). Read-only.",
    chatThreadClosedPlaceholder: "Conversation closed",

    // ChatSettingsPage - Missing strings
    chatSettingsTitle: "CHAT SETTINGS",
    muteSoundsLabel: "Mute sounds",
    blockNotificationsLabel: "Block notifications",
    simpleTestMessage: "Simulate incoming message (test)",
    membersTitle: "MEMBERS",
    addMemberHint: "+ Add member",
    inviteMembersTitle: "Invite a friend",
    noMoreFriendsToAdd: "All your friends are already in the group.",
    viewMyProfileLabel: "View my profile",
    viewProfileLabel: "View profile of",
    removeMemberConfirm:
      "Do you really want to remove this member from the group?",
    leaveGroupLabel: "Leave group",
    leaveConversationLabel: "Leave conversation",
    leaveGroupConfirmMessage: "Do you really want to leave this group?",
    leaveConversationConfirmMessage:
      "Do you really want to leave this conversation?",

    // CreateEventPage - Missing strings
    createEventTitle: "New event",
    editEventTitle: "Edit event",
    closeButton: "Close",
    createButton: "Create",
    saveBadgeButton: "✨ Save",
    createBadgeButton: "✨ Create event",
    coverThemeIntro: "Or image by theme",
    dateHint: "Date",
    timeHint: "Time",
    locationFieldLabel: "Location",
    locationFieldExample: "Ex: Central Park",
    increaseMaxButton: "Increase max",
    decreaseMaxButton: "Decrease max",
    descriptionFieldHint: "Add details, schedule, items to bring…",
    cancelButton: "Cancel",
    closeEventButton: "Close",
    cancelSortieConfirm:
      "Permanently cancel this event? It will disappear from the list and the discussion group will be deleted.",

    // OtherProfilePage - Missing strings
    otherProfileBio: "No bio yet.",
    profileVerifiedBadge: "Verified profile",
    memberSincePrefix: "Member since",
    statsReliability: "Reliability",
    statsEvents: "Events",
    statsFriends: "Friends",
    badgesSection: "Badges",
    favoritesAndHostedTab: "Favorites & Hosted",
    friendsTab: "Friends",
    pastTab: "Past",
    noUpcomingForProfile:
      "No upcoming event (favorite or hosted) linked to this profile. Past dates are in 'Past'.",
    noNetworkPreview: "No network preview for this profile.",
    noPastForProfile: "No past events linked to this profile.",
    viewButton: "View",
    organizerLabel: "Organizer",
    eventsInCommon: "in common",
  },
};

export type LanguageKey = keyof typeof translations;
export type TranslationKey = keyof typeof translations.fr;
