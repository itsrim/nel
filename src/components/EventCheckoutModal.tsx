...
export function EventCheckoutModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const result = await processEventCardPayment(eventId, {
        cardholderName,
        cardNumber,
        expiry,
        cvc,
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="event-checkout-title"
    >
      <div
      ...
      >
        <div className="sub-checkout-header">
          <div className="sub-checkout-plan">
            <span className="sub-checkout-plan-icon sub-checkout-plan-icon--premium">
              <Ticket size={20} />
            </span>
            <div>
              <h3 id="event-checkout-title">{t("eventCheckoutTitle")}</h3>
              <p>{eventTitle}</p>
            </div>
          </div>
